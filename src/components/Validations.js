...
export function validateFreeInput(nameControl, stringToValidate){
    ...
    else
    {
        ...
        else if(((isWrongWeight || isWrongCuporDrinkNumber) && (trimmedStringToValidate.length>1) ) || isWrongHeight)
        {
            return "Please introduce a valid "+nameControl;
        }
        else
...
export function validateDemographicData(demographicData){
    const errors=!demographicData.name ||
...
export function validatePainConditionData(numberOfPainConditions, painConditionData){
    ...
    const errors= numberOfPainConditions === 0 ||
